...
T_status blpp_Lzma_UncompressData(PBLPP_LZMA_UNCOMPRESS_REQUST pLUR)
{
    ...
    size_t inl,outl;
    int ir;
    PT_void WorkMem = NULL;
    bool bAlloc = false;
    if (NULL==pLUR || NULL==pLUR->CompressedData)
    {
        return T_STATUS_INVALID_PARAMETER;
    }
    if (pLUR->CompressedLength <= 13)
    {
        return T_STATUS_EXCLUDE;
    }
    if (NULL == pLUR->UncompressedData)
    {
        pLUR->UncompressedLength = (T_Bit32u)zzyCpEng_GetUncompressedSize(pLUR->CompressedData);
        pLUR->UncompressedData = (PT_byte)blpp_mem_alloc(pLUR->UncompressedLength);
        if (NULL == pLUR->UncompressedData)
        {
            return T_STATUS_INSUFFICIENT_RESOURCES;
        }
        bAlloc = true;
    }
    else
    {
        outl = (size_t)zzyCpEng_GetUncompressedSize(pLUR->CompressedData);
        if (pLUR->UncompressedLength < outl)
        {
            return T_STATUS_BUFFER_TOO_SMALL;
        }
    }
    //
    // Enlarge input if needed.
    //
    outl = pLUR->UncompressedLength;
    try
    {
        inl = pLUR->CompressedLength;
        ir = zzyCpEng_Uncompress(
            NULL,
            pLUR->UncompressedData,
            &outl,
            pLUR->CompressedData,
            &inl,
            tmpMemAlloc,
        ...
        if (SZ_OK == ir)
        {
            pLUR->UncompressedLength = (T_Bit32u)outl;
            Status = T_STATUS_SUCCESS;
        }
        else if (SZ_ERROR_INPUT_EOF == ir)
        {
            WorkMem = blpp_mem_alloc(inl);
            if (NULL == WorkMem)
            {
            ...
            {
                memcpy(WorkMem,pLUR->CompressedData,pLUR->CompressedLength);
                ir = zzyCpEng_Uncompress(
                    NULL,
                    pLUR->UncompressedData,
                    &outl,
    ...
    {
        Status = T_STATUS_UNKOWN_ERROR;
    }
    if (WorkMem)
    {
        blpp_mem_free(WorkMem);
    }
    if (!T_IsSuccess(Status) && bAlloc)
    {
        blpp_mem_free(pLUR->UncompressedData);
        pLUR->UncompressedData = NULL;
        pLUR->UncompressedLength = 0;
    }
    return Status;
}
...
T_status blpp_Lzma_CompressData(PBLPP_LZMA_COMPRESS_REQUST pLCR)
{
    ...
    size_t outl;
    T_Bit32u loop = 0;
    PT_byte tempMem = NULL;
    int ir;
    if (pLCR->CompressedData)
    {
        outl = pLCR->CompressedLength;
        try
        {
            ir = zzyCpEng_Compress(
                NULL,
                pLCR->CompressedData,
                &outl,
                pLCR->InputData,
            ...
            if (SZ_OK == ir)
            {
                pLCR->CompressedLength = (T_Bit32u)outl;
                Status = T_STATUS_SUCCESS;
            }
            else if (SZ_ERROR_OUTPUT_EOF == ir)
            {
                Status = T_STATUS_BUFFER_TOO_SMALL;
            }
            else
            {
                Status = T_STATUS_DATA_ERROR;
    ...
    try
    {
        while ((loop++) < BLPP_LZMA_TRY_TIMES)
        {
            outl = pLCR->InputLength+(loop*16*1024); // plus 16KB.
            tempMem = (PT_byte)blpp_mem_alloc(outl);
            if (NULL == tempMem)
            {
                return T_STATUS_INSUFFICIENT_RESOURCES;
            }
            ...
            if (SZ_OK == ir)
            {
                pLCR->CompressedData = (PT_byte)blpp_mem_alloc(outl);
                if (NULL == pLCR->CompressedData)
                {
                    pLCR->CompressedLength = 0;
                    blpp_mem_free(tempMem);
                    return T_STATUS_INSUFFICIENT_RESOURCES;
                }
                memcpy(pLCR->CompressedData,tempMem,outl);
                pLCR->CompressedLength = (T_Bit32u)outl;
                Status = T_STATUS_SUCCESS;
                break;
            }
    ...
    if (!T_IsSuccess(Status) && pLCR->CompressedData)
    {
        blpp_mem_free(pLCR->CompressedData);
        pLCR->CompressedData = NULL;
        pLCR->CompressedLength = 0;
    }
    return Status;
}
...
{
    int ir = zzyCpEng_StreamCompress(lparam,allLength,pRead,pWrite,level,0,-1,-1,-1,-1,tmpMemAlloc,tmpMemFree);
    switch (ir)
    {
    case SZ_OK:
