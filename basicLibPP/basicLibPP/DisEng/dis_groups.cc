...
void disassembler::Apw(const x86_insn *insn)
{
  ...
  pOperand->UseSelector = 1;
  pOperand->cs_selector = cs_selector;
  pOperand->ImmSize = NS_16u;
  pOperand->imm.immu = imm16;
...
void disassembler::Apd(const x86_insn *insn)
{
  ...
  pOperand->UseSelector = 1;
  pOperand->cs_selector = cs_selector;
  pOperand->ImmSize = NS_32u;
  pOperand->imm.immu = imm32;
  dis_sprintf("0x%04x:%08x", (unsigned) cs_selector, (unsigned) imm32);
}

// 8-bit general purpose registers
void disassembler::AL_Reg(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_GENERAL_REG);
  pOperand->GeneralRegSize = RS_8l;
  pOperand->GeneralReg = R_RAX;
  dis_sprintf("%s", general_8bit_regname[rAX_REG]);
}
void disassembler::CL_Reg(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_GENERAL_REG);
  pOperand->GeneralRegSize = RS_8l;
  pOperand->GeneralReg = R_RCX;
  dis_sprintf("%s", general_8bit_regname[rCX_REG]);
}

// 16-bit general purpose registers
void disassembler::AX_Reg(const x86_insn *insn) {
  SetFlag(pOperand->RegType,DisEng_USE_GENERAL_REG);
  pOperand->GeneralRegSize = RS_16;
  pOperand->GeneralReg = R_RAX;
...
// segment registers
void disassembler::CS(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_SEGMENT_REG);
  pOperand->SegmentReg = R_CS;
  dis_sprintf("%s", segment_name[CS_REG]);
}
void disassembler::DS(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_SEGMENT_REG);
  pOperand->SegmentReg = R_DS;
  dis_sprintf("%s", segment_name[DS_REG]);
}
void disassembler::ES(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_SEGMENT_REG);
  pOperand->SegmentReg = R_ES;
  dis_sprintf("%s", segment_name[ES_REG]);
}
void disassembler::SS(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_SEGMENT_REG);
  pOperand->SegmentReg = R_SS;
  dis_sprintf("%s", segment_name[SS_REG]);
}
void disassembler::FS(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_SEGMENT_REG);
  pOperand->SegmentReg = R_FS;
  dis_sprintf("%s", segment_name[FS_REG]);
}
void disassembler::GS(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_SEGMENT_REG);
  pOperand->SegmentReg = R_GS;
  dis_sprintf("%s", segment_name[GS_REG]);
}

void disassembler::Sw(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_SEGMENT_REG);
  pOperand->SegmentReg = (DisEng_SegmentReg)insn->nnn;
  dis_sprintf("%s", segment_name[insn->nnn]);
}

// control register
void disassembler::Cd(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_CONTROL_REG);
  pOperand->ControlRegNumber = insn->nnn;
  if (intel_mode)
...
{
  SetFlag(pOperand->RegType,DisEng_USE_DEBUG_REG);
  pOperand->DebugRegNumber = insn->nnn;
  if (intel_mode)
...
void disassembler::Reg8(const x86_insn *insn)
{
  ...
  {
	pOperand->GeneralRegSize = RS_8l;
	pOperand->GeneralReg = (DisEng_GeneralReg)reg;
    dis_sprintf("%s", general_8bit_regname_rex[reg]);
  }
  else
  {
	pOperand->GeneralRegSize = RS_8h;
	pOperand->GeneralReg = (DisEng_GeneralReg)(reg-4);
...
void disassembler::RX(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_GENERAL_REG);
  pOperand->GeneralRegSize = RS_16;
  pOperand->GeneralReg = (DisEng_GeneralReg)((insn->b1 & 7) | insn->rex_b);
...
void disassembler::Eb(const x86_insn *insn)
{
  if (insn->mod == 3) {
	...
	{
	  pOperand->GeneralRegSize = RS_8l;
	  pOperand->GeneralReg = (DisEng_GeneralReg)insn->rm;
      dis_sprintf("%s", general_8bit_regname_rex[insn->rm]);
	}
    else
	{
	  pOperand->GeneralRegSize = RS_8h;
	  pOperand->GeneralReg = (DisEng_GeneralReg)(insn->rm-4);
...
void disassembler::Gb(const x86_insn *insn)
{
  ...
  {
	pOperand->GeneralRegSize = RS_8l;
	pOperand->GeneralReg = (DisEng_GeneralReg)insn->nnn;
    dis_sprintf("%s", general_8bit_regname_rex[insn->nnn]);
  }
  else
  {
	pOperand->GeneralRegSize = RS_8h;
	pOperand->GeneralReg = (DisEng_GeneralReg)(insn->nnn-4);
...
void disassembler::By(const x86_insn *insn)
{
  ...
  {
	pOperand->GeneralRegSize = RS_64;
	pOperand->GeneralReg = (DisEng_GeneralReg)insn->vex_vvv;
...
void disassembler::Ib(const x86_insn *insn)
{
  Bit8u ib = fetch_byte();
  pOperand->ImmSize = NS_8u;
  pOperand->imm.immu = ib;
  if (! intel_mode) dis_putc('$');
  dis_sprintf("0x%02x", (unsigned) ib);
}
...
void disassembler::Iw(const x86_insn *insn)
{
  ...
  pOperand->ImmSize = NS_16u;
  pOperand->imm.immu = iw;
  if (! intel_mode) dis_putc('$');
  dis_sprintf("0x%04x", (unsigned) iw);
}
...
void disassembler::IbIb(const x86_insn *insn)
{
  ...
  pOperand->ImmSize = NS_8u;
  pOperand->imm.immu = ib1;
  ++pOperand;
  pOperand->ImmSize = NS_8u;
  pOperand->imm.immu = ib2;
  if (intel_mode) {
...
{
  Bit32u value = fetch_dword();

  pOperand->ImmSize = NS_32u;
  pOperand->imm.immu = value;
  if (! intel_mode) dis_putc('$');
  dis_sprintf("0x%08x", (unsigned) value);
}
...
{
  SetFlag(pOperand->RegType,DisEng_USE_FLOATING_POINT_REG);
  pOperand->FloatingPointRegNumber = 0;
  if (intel_mode)
    dis_sprintf  ("st(0)");
  else
    dis_sprintf("%%st(0)");
...
{
  SetFlag(pOperand->RegType,DisEng_USE_FLOATING_POINT_REG);
  pOperand->FloatingPointRegNumber = insn->rm & 7;
  if (intel_mode)
...
{
  SetFlag(pOperand->RegType,DisEng_USE_MMX_REG);
  pOperand->MMXRegNumber = insn->nnn & 0x7;
  if (intel_mode)
...
{
  SetFlag(pOperand->RegType,DisEng_USE_MMX_REG);
  pOperand->MMXRegNumber = insn->rm & 0x7;
  if (intel_mode)
...
void disassembler::Udq(const x86_insn *insn)
{
  ...
  pOperand->VectorRegNumber = insn->rm;
...
void disassembler::Vq(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_VECTOR_REG);
  pOperand->VectorReg = (DisEng_VectorReg)insn->vex_l;
  pOperand->VectorRegNumber = insn->nnn;
...
void disassembler::Hdq(const x86_insn *insn)
{
  SetFlag(pOperand->RegType,DisEng_USE_VECTOR_REG);
  pOperand->VectorReg = (DisEng_VectorReg)insn->vex_l;
  pOperand->VectorRegNumber = insn->vex_vvv;
...
void disassembler::OP_X(const x86_insn *insn, unsigned size)
{
  ...
  SetFlag(pOperand->RegType,DisEng_USE_SEGMENT_REG|DisEng_USE_GENERAL_REG);
  pOperand->GeneralReg = R_RSI;
  if (insn->as_64) {
	pOperand->GeneralRegSize = RS_64;
...
void disassembler::Jb(const x86_insn *insn)
{
  ...
  if (insn->os_32) {
    ...
    if (db_cs_base != BX_JUMP_TARGET_NOT_REQ) {
	  ...
	  pOperand->JumpRealAddr = target + db_cs_base;
